...
class MercadoLibreConnectionAccount(models.Model):
    ...
    type = fields.Selection([("mercadolibre","MercadoLibre")],string='Connector Type',default="mercadolibre", index=True, required=True)
                ...
                if seller_sku:
                    ...
                    if type(seller_sku) in (tuple, list):
                        ...
                        for sku in seller_sku:
                            ...
                            sku_product_id = self.env['product.product'].search([ ('default_code','=',sku) ])
                            if sku_product_ids:
                                sku_product_ids+= sku_product_id
                            else:
                                sku_product_ids = sku_product_id
                        sku_product_id = sku_product_ids
                    else:
                        sku_product_id = self.env['product.product'].search([ ('default_code','=',seller_sku) ])
...
#MELI internal
    ...
    def product_meli_get_products(self, limit=50):
        #
        account = self
        _logger.info('account.product_meli_get_products() '+str(account.name))
        company = account.company_id or self.env.user.company_id
        ...
        response = meli.get( url_get, { 'access_token':meli.access_token, 'offset': 0, 'limit': limit })
        rjson = response.json()
        ...
        if (totalmax<=1000 and len(results)<totalmax and ('paging' in rjson and totalmax>rjson['paging']['limit']) ):
            ...
            while (condition_last_off!=True):
                _logger.info( "Prefetch products ("+str(ioff)+"/"+str(rjson['paging']['total'])+")" )
                response = meli.get("/users/"+str(account.seller_id)+"/items/search", {'access_token':meli.access_token, 'offset': ioff, 'limit': limit }) #?"+urlencode({'offset': ioff })
                rjson2 = response.json()
                if 'error' in rjson2:
                    if rjson2['message']=='invalid_token' or rjson2['message']=='expired_token':
                        ACCESS_TOKEN = ''
    ...
    def meli_update_remote_price(self, meli=None):
        ...
        account = self
        _logger.info('account.meli_update_remote_price() '+str(account.name))
        company = account.company_id or self.env.user.company_id
        config = account.configuration or company
